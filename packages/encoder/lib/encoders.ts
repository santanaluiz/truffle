...
export class ProjectEncoder {
  ...
  private referenceDeclarations: { [compilationId: string]: Codec.Ast.AstNodes };
   ...
   * `"byte"` are accepted.  Also, user-defined value types use exactly the same
  ...
  public async forAddress(
    ...
    const deployedBytecode = await new ProviderAdapter(this.provider).getCode(address, block);
...
export class ContractEncoder {
  ...
  constructor(
    ...
    const links = networkId !== null
      ? (((artifact || {networks: {}}).networks || {})[networkId] || {links: {}}).links || {}
      : {};
  ...
  public async encodeTxNoResolution(
    ...
    const method = this.getMethod(abi);
    return await this.projectEncoder.encodeTxNoResolution(method, inputs, options);
  }
  ...
  public async encodeTransaction(
    ...
    //write abis.map(this.getMethod.bind(this)), but I find the
    //arrow way to be more readable
    return await this.projectEncoder.encodeTransaction(methods, inputs, options);
  }
  ...
  public async forInstance(address?: string): Promise<ContractInstanceEncoder> {
    if (address === undefined) {
      ...
      address = ((this.artifact || {networks: {}}).networks || {})[networkId].address;
      if (address === undefined) {
  ...
  private getAbis(
    ...
    const abis: Abi.FunctionEntry[] = typeof abisOrName === "string"
      ? this.abi.filter(
    ...
    if (typeof abisOrName === "string" && abis.length === 0) {
      ...
      //did that :P
      throw new NoFunctionByThatNameError(abisOrName, this.contract.contractName);
    }
    return abis;
  };
